(function () {
    ...
    function initializeTeamCalendar(retryCount) {
        ...
        if (!elements.page) {
            if (retryCount < 20) {
                setTimeout(function () {
                    initializeTeamCalendar(retryCount + 1);
                }, 150);
            }
            return;
        }
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initializeTeamCalendar);
    } else {
        initializeTeamCalendar();
    }

    function cacheElements() {
        elements.page = document.querySelector(".calendarPage--team") || document.querySelector('.calendarPage[data-calendar-scope="team"]');
        ...
        elements.visibilityInput = document.querySelector(".calendarVisibilitySelect");
        ...
        elements.visibilityField = elements.visibilityInput ? elements.visibilityInput.closest(".calendarField") : null;
    ...
    function bindEvents() {
        ...
        if (elements.addButton) elements.addButton.addEventListener("click", function () { openEventModal(null, state.selectedDate); });
    ...
    async function loadEvents() {
        try {
            ...
            state.events = Array.isArray(data.items) ? data.items.map(normalizeEvent).filter(function (item) {
                ...
                return item.calendarScope === CALENDAR_SCOPE && item.visibility !== "private" && (canReadAllTeamCalendars() || item.department === getUserDepartment());
            }) : [];
        } catch (error) {
            state.events = [];
        }
    ...
    function isVacationCalendarEvent(item) {
        ...
        return /^approval_vacation_/i.test(id) || /연차|월차|반차|휴가|휴무|부재/.test(title);
    }
    ...
    function isCompanyWideEvent(item) {
        return isCompanyWideScope(item && (item.calendarScope || item.scope || item.visibility));
    }

    function isCompanyWideScope(value) {
        var scope = normalizeCalendarScope(value);
        return scope === "wide" || scope === "company" || scope === "company-wide" || scope === "global" || scope === "all";
    }
    ...
    function openEventModal(item, fallbackDate) {
        ...
        var editable = !editing || canEditEvent(item);
        ...
        setLabelColorValue(editing ? item.labelColor || DEFAULT_LABEL_COLOR : DEFAULT_LABEL_COLOR);
        setVisibilityValue(editing ? item.visibility || "team" : "team");
    ...
    async function saveEvent() {
        ...
        var visibility = getVisibilityValue();
        var previousItem = id ? getEventById(id) : null;
        var previousVisibility = previousItem ? previousItem.visibility || "team" : "";
        ...
        var payload = normalizeEvent({
            ...
            department: previousItem && previousItem.department || getUserDepartment(),
            createdAt: previousItem && previousItem.createdAt || new Date().toISOString(),
            updatedAt: new Date().toISOString(),
            calendarScope: visibility === "team" ? CALENDAR_SCOPE : "my"
        });
        ...
        try {
            if (previousVisibility && previousVisibility !== visibility) {
                await removeEventFromTarget(payload.id, previousVisibility);
            }
            await saveEventToTarget(payload, visibility);
            await loadEvents();
            closeEventModal();
            renderCalendar();
        } catch (error) {
    ...
    async function deleteEvent() {
        ...
        try {
            ...
            await removeEventFromTarget(id, item && item.visibility || "team");
            await loadEvents();
            closeEventModal();
            renderCalendar();
        } catch (error) {
    ...
    async function saveEventToTarget(payload, visibility) {
        if (visibility === "team") {
            await saveSharedEvent(payload);
            return;
        }
        savePrivateEvent(payload);
    }

    async function removeEventFromTarget(id, visibility) {
        if (visibility === "team") {
            await deleteSharedEvent(id);
            return;
        }
        deletePrivateEvent(id);
    }

    function savePrivateEvent(payload) {
        var list = getPrivateEvents();
        var index = list.findIndex(function (item) { return item.id === payload.id; });
        if (index > -1) list[index] = payload;
        else list.push(payload);
        localStorage.setItem(getPrivateStorageKey(), JSON.stringify(list));
    }

    function deletePrivateEvent(id) {
        var list = getPrivateEvents().filter(function (item) {
            return item.id !== id;
        });
        localStorage.setItem(getPrivateStorageKey(), JSON.stringify(list));
    }
    ...
    function getPrivateEvents() {
        try {
            var saved = localStorage.getItem(getPrivateStorageKey());
    ...
    function getEventById(id) {
        return state.events.find(function (item) { return item.id === id; }) || null;
    }

    function setModalEditable(editable, editing) {
        var textInputs = [elements.titleInput, elements.startInput, elements.endInput, elements.startTimeInput, elements.endTimeInput, elements.locationInput, elements.memoInput, elements.alertInput, elements.visibilityInput, elements.startYearSelect, elements.startMonthSelect, elements.startDaySelect, elements.endYearSelect, elements.endMonthSelect, elements.endDaySelect, elements.startTimeSelect, elements.endTimeSelect];
        ...
        if (elements.allDayInput) elements.allDayInput.disabled = !editable;
        elements.labelInputs.forEach(function (input) { input.disabled = !editable; });
        if (elements.deleteButton) elements.deleteButton.style.display = editable && editing ? "inline-flex" : "none";
        if (elements.saveButton) {
            elements.saveButton.style.display = editable ? "inline-flex" : "none";
        }
        toggleFormFields(editable || !editing);
    }
    ...
    function ensureReadonlyDetail() {
        ...
        container.innerHTML = ''
            ...
            + '    <div class="calendarReadonlyTitle"></div>'
            + '    <div class="calendarReadonlyDateLine">'
            + '      <span class="calendarReadonlyDate"></span>'
            + '      <span class="calendarReadonlyTime"></span>'
            + '    </div>'
            + '  </div>'
            + '</div>'
            + '<div class="calendarReadonlySection calendarReadonlyLocation">'
            + '  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>'
            + '  <p></p>'
            + '</div>'
            + '<div class="calendarReadonlySection calendarReadonlyMemo">'
            + '  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M12 2v4"/><path d="M16 2v4"/><rect width="16" height="18" x="4" y="4" rx="2"/><path d="M8 10h6"/><path d="M8 14h8"/><path d="M8 18h5"/></svg>'
            + '  <p></p>'
            + '</div>'
            + '<div class="calendarReadonlyMeta">'
            + '  <div class="calendarReadonlySection calendarReadonlyAlert">'
            ...
            + '    <p></p>'
            + '  </div>'
            + '  <div class="calendarReadonlySection calendarReadonlyVisibility">'
            + '    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/></svg>'
            + '    <p></p>'
            + '  </div>'
            + '</div>';
        ...
        elements.readonlyDate = container.querySelector(".calendarReadonlyDate");
        elements.readonlyTime = container.querySelector(".calendarReadonlyTime");
        elements.readonlyLocation = container.querySelector(".calendarReadonlyLocation");
        elements.readonlyLocationText = container.querySelector(".calendarReadonlyLocation p");
        elements.readonlyMemo = container.querySelector(".calendarReadonlyMemo");
        elements.readonlyMemoText = container.querySelector(".calendarReadonlyMemo p");
        elements.readonlyAlert = container.querySelector(".calendarReadonlyAlert");
        elements.readonlyAlertText = container.querySelector(".calendarReadonlyAlert p");
        elements.readonlyVisibility = container.querySelector(".calendarReadonlyVisibility");
        elements.readonlyVisibilityText = container.querySelector(".calendarReadonlyVisibility p");
    }

    function ensureReadonlyAuthorBadge() {
        if (!elements.modalHead || elements.readonlyAuthorBadge) return;
        var badge = document.createElement("div");
        badge.className = "calendarReadonlyAuthor";
        badge.style.display = "none";
        elements.modalHead.insertBefore(badge, elements.closeButton || null);
        elements.readonlyAuthorBadge = badge;
    }

    function renderReadonlyDetail(item) {
        ensureReadonlyDetail();
        if (!elements.readonlyDetail || !item) return;
        if (elements.readonlyAuthorBadge) {
            elements.readonlyAuthorBadge.classList.toggle("is-vacation", isVacationCalendarEvent(item));
            elements.readonlyAuthorBadge.textContent = isCompanyWideEvent(item) ? "전사 일정" : item.createdByName || item.createdById || "작성자";
            elements.readonlyAuthorBadge.style.background = isCompanyWideEvent(item) ? WIDE_LABEL_COLOR : sanitizeColor(item.labelColor);
        }
        if (elements.readonlyTitle) elements.readonlyTitle.textContent = item.title || "-";
        if (elements.readonlyDate) elements.readonlyDate.textContent = formatReadonlyDateRange(item.startDate, item.endDate);

        var timeText = item.allDay ? "" : formatReadonlyTimeRange(item.startTime, item.endTime);
        if (elements.readonlyTime) {
            elements.readonlyTime.textContent = timeText;
            elements.readonlyTime.style.display = timeText ? "" : "none";
        }

        if (elements.readonlyLocation && elements.readonlyLocationText) {
            elements.readonlyLocationText.textContent = item.location || "";
            elements.readonlyLocation.style.display = item.location ? "" : "none";
        }

        if (elements.readonlyMemo && elements.readonlyMemoText) {
            var memoText = isVacationCalendarEvent(item) ? formatVacationMemoLine(item) : item.memo || "";
            elements.readonlyMemoText.textContent = memoText;
            elements.readonlyMemo.style.display = memoText ? "" : "none";
        }

        if (elements.readonlyAlert && elements.readonlyAlertText) {
            elements.readonlyAlertText.textContent = formatAlertText(item.alert);
        }
    ...
    function toggleReadonlyDetail(show) {
        ...
        if (elements.readonlyAuthorBadge) elements.readonlyAuthorBadge.style.display = show ? "inline-flex" : "none";
    }
    ...
    function getVisibilityValue() {
        return elements.visibilityInput ? String(elements.visibilityInput.value || "team").trim() : "team";
    }
    ...
    function setVisibilityValue(value) {
        if (!elements.visibilityInput) return;
        elements.visibilityInput.value = value === "private" ? "private" : "team";
    }
    ...
    function getUserDepartment() {
        return normalizeDepartment(String(state.user && state.user.department || "").trim());
    }

    function canReadAllTeamCalendars() {
        if (window.AuthStore && typeof window.AuthStore.canReadAllTeamCalendars === "function") {
            return !!window.AuthStore.canReadAllTeamCalendars();
        }
        var role = String(state.user && state.user.role || localStorage.getItem("userRole") || "").trim().toLowerCase();
        var department = getUserDepartment();
        return role === "admin" || role === "ceo" || role === "관리자" || role === "대표" || department === "대표";
    }

    function getPrivateStorageKey() {
        return "calendarEvents:my:" + getUserId();
    }
    ...
    function normalizeEvent(item) {
        return {
            ...
            visibility: item && item.visibility === "private" ? "private" : "team",
            ...
            requesterId: String(item && item.requesterId || "").trim().toLowerCase(),
            ownerId: String(item && item.ownerId || "").trim().toLowerCase(),
            authorId: String(item && item.authorId || "").trim().toLowerCase(),
            userId: String(item && item.userId || "").trim().toLowerCase(),
    ...
    function formatReadonlyDateRange(startDate, endDate) {
        var startText = formatReadonlyDate(startDate);
        var endText = formatReadonlyDate(endDate || startDate);
        if (!startText) return "";
    ...
    function formatReadonlyTimeRange(startTime, endTime) {
        return formatDisplayTimeRange(startTime, endTime);
    }

    function formatAlertText(value) {
        var alertValue = String(value || "none").trim();
        var labels = {
            "none": "알림 안 함",
            "at-time": "정시 알림",
            "10-min": "10분 전 알림",
        ...
        return labels[alertValue] || "알림 안 함";
    }

    function sanitizeColor(value) {
        var color = String(value || "").trim();
        return /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(color) ? color : DEFAULT_LABEL_COLOR;
    }
