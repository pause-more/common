...
(function () {
    ...
    function cacheElements() {
        elements.page = document.querySelector(".calendarPage--my");
        if (!elements.page) return;
        ...
        elements.modalTitle = document.querySelector(".calendarModalTitle");
    ...
    function bindEvents() {
        ...
        if (elements.closeButton) {
            elements.closeButton.addEventListener("click", closeEventModal);
        }
        if (elements.modalDim) {
            elements.modalDim.addEventListener("click", closeEventModal);
        }
        if (elements.saveButton) {
            elements.saveButton.addEventListener("click", saveEvent);
        }
        if (elements.deleteButton) {
            elements.deleteButton.addEventListener("click", deleteEvent);
        }
        if (elements.allDayInput) {
            elements.allDayInput.addEventListener("change", syncAllDayFields);
        }
    ...
    async function loadEvents() {
        var privateEvents = getPrivateEvents();
        var sharedEvents = await getMySharedEvents();
        state.events = privateEvents.concat(sharedEvents).sort(function (a, b) {
            return String(a.startDate || "").localeCompare(String(b.startDate || "")) || String(a.title || "").localeCompare(String(b.title || ""), "ko");
        });
    }
    ...
    function getPrivateEvents() {
        try {
            ...
            return Array.isArray(parsed) ? parsed.map(function (item) { return normalizeEvent(item, "private"); }) : [];
        } catch (error) {
            return [];
        }
    }

    async function getMySharedEvents() {
        try {
            var data = await API.get(SHARED_API_BASE, null, {
                errorMessage: "공유 일정을 불러오지 못했습니다."
            });
            if (!data.success || !Array.isArray(data.items)) return [];
            return data.items.map(function (item) {
                return normalizeEvent(item, "shared");
            }).filter(function (item) {
                return isCompanyWideEvent(item) || (item.createdById === getUserId() && (!item.calendarScope || item.calendarScope === "team" || item.calendarScope === "shared"));
            });
        } catch (error) {
            return [];
        }
    ...
    function openEventModal(item, fallbackDate) {
        ...
        if (elements.modalTitle) elements.modalTitle.textContent = editing ? (editable ? "일정 수정" : "일정 확인") : "일정 등록";
        ...
        if (elements.allDayInput) elements.allDayInput.checked = editing ? item.allDay !== false : false;
        ...
        setVisibilityValue(editing ? item.visibility || "private" : "private");
        syncAllDayFields();
        state.modalReadonly = !editable;
        setModalEditable(editable, editing);
        if (!editable && editing) {
            elements.modal.classList.add("is-readonly-view");
    ...
    async function saveEvent() {
        ...
        var previousVisibility = previousItem ? itemVisibilityToStorage(previousItem.visibility || "private") : "";

        if (previousItem && !canEditEvent(previousItem)) {
            alert("전사 일정은 확인만 할 수 있습니다.");
            return;
        }
        ...
        var payload = normalizeEvent({
            ...
            calendarScope: itemVisibilityToStorage(visibility) === "shared" ? "team" : "my"
        }, visibility);
    ...
    async function deleteEvent() {
        ...
        if (item && !canEditEvent(item)) {
            alert("전사 일정은 삭제할 수 없습니다.");
            return;
        }
        if (!confirm("일정을 삭제할까요?")) return;
        try {
            await removeEventFromTarget(id, itemVisibilityToStorage(item && item.visibility || "private"));
            await loadEvents();
            closeEventModal();
            renderCalendar();
        } catch (error) {
    ...
    async function saveEventToTarget(payload, visibility) {
        if (itemVisibilityToStorage(visibility) === "shared") {
    ...
    async function saveSharedEvent(payload) {
        await API.post(SHARED_API_BASE + "/save", payload, {
            errorMessage: "공유 일정을 저장하지 못했습니다."
        });
    }
    ...
    async function deleteSharedEvent(id) {
        await API.post(SHARED_API_BASE + "/delete", {
                ...
                errorMessage: "공유 일정을 삭제하지 못했습니다."
        });
    }
    ...
    function canEditEvent(item) {
        ...
        if (isCompanyWideEvent(item)) return false;
        if (itemVisibilityToStorage(item.visibility) === "private") return true;
    ...
    function setModalEditable(editable, editing) {
        var inputs = [
            ...
            elements.startMonthSelect,
            elements.startDaySelect,
            elements.endYearSelect,
            elements.endMonthSelect,
            elements.endDaySelect,
            elements.startTimeSelect,
            elements.endTimeSelect
        ];
        inputs.forEach(function (input) {
            if (!input) return;
            input.disabled = !editable;
            input.readOnly = !editable && input.tagName !== "SELECT";
        ...
        if (elements.saveButton) elements.saveButton.style.display = editable ? "inline-flex" : "none";
        if (elements.deleteButton) elements.deleteButton.style.display = editable && editing ? "inline-flex" : "none";
        if (elements.actions) elements.actions.style.display = editable || !editing ? "" : "none";
    }
    ...
    function renderReadonlyDetail(item) {
        ...
        if (elements.readonlyAuthor) {
            ...
            elements.readonlyAuthor.textContent = isCompanyWideEvent(item) ? "전사 일정" : item.createdByName || item.createdById || "작성자";
            elements.readonlyAuthor.style.background = isCompanyWideEvent(item) ? WIDE_LABEL_COLOR : sanitizeColor(item.labelColor);
        }
        ...
        if (elements.readonlyTimeText) {
            var timeText = item.allDay ? "" : formatEventTimeRange(item);
        ...
        elements.readonlyDetail.classList.toggle("is-body-empty", sections.length === 0);
    ...
    function buildReadonlyMeta(item) {
        return [
            ...
            buildReadonlySection(iconCalendar(), isCompanyWideEvent(item) ? "전사 공개" : "내 캘린더")
        ];
    }
    ...
    function iconBell() {
        return '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M10.268 21a2 2 0 0 0 3.464 0"></path><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326"></path></svg>';
    }

    function iconCalendar() {
        return '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"></path><path d="M16 2v4"></path><rect width="18" height="18" x="3" y="4" rx="2"></rect><path d="M3 10h18"></path></svg>';
    }

    function iconLocation() {
        return '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"></path><circle cx="12" cy="10" r="3"></circle></svg>';
    }

    function iconMemo() {
        return '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"></path><path d="M12 2v4"></path><path d="M16 2v4"></path><rect width="16" height="18" x="4" y="4" rx="2"></rect><path d="M8 10h6"></path><path d="M8 14h8"></path><path d="M8 18h5"></path></svg>';
    }

    function setVisibilityValue(value) {
        if (!elements.visibilityInput) return;
        elements.visibilityInput.value = value === "shared" || value === "team" ? "team" : "private";
    }
    ...
    function getVisibilityValue() {
        return elements.visibilityInput ? String(elements.visibilityInput.value || "private").trim() : "private";
    }

    function getPrivateStorageKey() {
        return STORAGE_PREFIX + ":my:" + getUserId();
    }
    ...
    function normalizeEvent(item, fallbackVisibility) {
        return {
            ...
            visibility: itemVisibilityToStorage(item && item.visibility) || itemVisibilityToStorage(fallbackVisibility) || "private",
            createdById: String(item && (item.createdById || item.requesterId) || getUserId()).trim().toLowerCase(),
            createdByName: String(item && (item.createdByName || item.requesterName) || getUserName()).trim(),
            department: normalizeDepartment(item && item.department || getUserDepartment()),
            createdAt: String(item && item.createdAt || "").trim(),
            updatedAt: String(item && item.updatedAt || "").trim(),
            calendarScope: normalizeCalendarScope(item && (item.calendarScope || item.scope) || (itemVisibilityToStorage(item && item.visibility) === "shared" ? "team" : "my"))
        };
    }
    ...
    function itemVisibilityToStorage(value) {
        var visibility = String(value || "").trim();
        return visibility === "shared" || visibility === "team" ? "shared" : "private";
    }
