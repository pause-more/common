...
var API = window.GroupwareApi;
    ...
    document.addEventListener("DOMContentLoaded", function () {
        initializeApprovalReturnPage();
    });

    async function initializeApprovalReturnPage() {
    ...
    async function loadReturnDocuments() {
        var tbody = document.querySelector(".approvalReturnListBody");
        if (!tbody) return;

        if (!state.user || !state.user.id) {
            renderReturnEmpty("로그인 정보가 없습니다.");
            return;
        }
        ...
        try {
            ...
            }, { errorMessage: "반려문서를 불러오지 못했습니다." });
            state.items = Array.isArray(data.items) ? data.items : [];
            renderReturnDocuments();
        } catch (error) {
            renderReturnEmpty(error.message || "반려문서를 불러오지 못했습니다.");
        }
    }

    function renderReturnDocuments() {
        var tbody = document.querySelector(".approvalReturnListBody");
        if (!tbody) return;

        if (!state.items.length) {
            renderReturnEmpty("반려문서가 없습니다.");
            return;
        }

        tbody.innerHTML = state.items.map(function (item) {
            return '<tr class="approvalListRow approvalReturnListRow" data-id="' + escapeApprovalHtml(item.id || "") + '">' +
                ...
                '<td>' + escapeApprovalHtml(formatApprovalDateTime(item.rejectedAt || item.decidedAt || item.updatedAt || "")) + '</td>' +
                '<td><span class="approvalStatusBadge rejected">반려</span></td>' +
            '</tr>';
        }).join("");

        tbody.querySelectorAll(".approvalReturnListRow").forEach(function (row) {
            row.addEventListener("click", function () {
                var id = row.getAttribute("data-id") || "";
                if (id) location.href = "/approval/detail.html?id=" + encodeURIComponent(id) + "&from=return";
            });
        });
    }

    function renderReturnEmpty(message) {
        var tbody = document.querySelector(".approvalReturnListBody");
        if (!tbody) return;
        tbody.innerHTML = '<tr><td colspan="6" class="approvalListEmpty">' + escapeApprovalHtml(message || "반려문서가 없습니다.") + '</td></tr>';
    }
    ...
    function renderApprovalDetail(item) {
        ...
        body.innerHTML =
            '<div class="approvalDetailGrid">' +
                ...
                renderDetailPair("반려일시", formatApprovalDateTime(item.rejectedAt || item.decidedAt || item.updatedAt || "")) +
            '</div>' +
            renderDetailApprovalLineCards(item) +
            renderDetailTextSection("반려사유", item.decisionReason) +
