...
var API = window.GroupwareApi;
    ...
    function renderClosedEmpty(message) {
        var tbody = document.querySelector(".approvalClosedListBody");
        if (!tbody) return;
        tbody.innerHTML = '<tr><td colspan="6" class="approvalListEmpty">' + escapeApprovalHtml(message || "결재완료 문서가 없습니다.") + '</td></tr>';
    }
    ...
    function renderDetailFiles(item) {
        ...
        return '<div class="approvalDetailSection"><h5>첨부파일</h5><ul class="approvalDetailFileList">' + files.map(function (file) {
            return '<li><a href="' + escapeApprovalHtml(file.url || "#") + '" target="_blank" rel="noopener">' + escapeApprovalHtml(file.name || "첨부파일") + '</a></li>';
        }).join("") + '</ul></div>';
    }

    function getProposalTitle(docType) {
        if (docType === "출장신청서") return "출장목적";
        if (docType === "휴가원") return "휴가사유";
        return "품의내용";
    }
    ...
    function formatDetailDate(value) {
        ...
        return date.getFullYear() + "." + padApprovalValue(date.getMonth() + 1) + "." + padApprovalValue(date.getDate());
    }

    function formatDetailPeriod(start, end) {
        var startText = formatDetailDate(start);
        var endText = formatDetailDate(end);
